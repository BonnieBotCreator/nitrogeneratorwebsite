(function(){
    var oldLog = console.log;
    console.log = function (message) {
        document.getElementById('a').value += message
        oldLog.apply(arguments);
    };
})();//Black Code

function download(filename, text) {
  var element = document.createElement('a');
  element.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent(text));//Black Code
  element.setAttribute('download', filename);

  element.style.display = 'none';
  document.body.appendChild(element);//Black Code

  element.click();

  document.body.removeChild(element);
}
var textarea = document.getElementById('a')
if (navigator.userAgent.indexOf('Mobile') !== -1) { document.getElementById('save').style.display = "none"; textarea.style.height = "100px";
}////Black Code
function generate() {
var triesPerSecond = document.getElementById('speed').value //Black Code
getGiftCode = function () {
    let code = '';
    let dict = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
    for(var i = 0; i < 24; i++){
        code = code + dict.charAt(Math.floor(Math.random() * dict.length));
    }
    console.log(' [KOD] http://discord.gift/' + code + "\n");
    code += '\n'
    document.getElementById('b').value += code;

} ////Black Code


getGiftCode();
document.getElementById('stop').addEventListener("click", stop); ////Black Code
function stop() {
  clearInterval(gInterval);
  clearInterval(interval)
} //Black Code
var gInterval = setInterval(() => {getGiftCode();}, (1/triesPerSecond) * -100000);
////Black Code

function scroll() {
  document.getElementById("a").scrollTop = document.getElementById("a").scrollHeight
}; //Black Code
var interval = setInterval(scroll, 100) //Black Code
document.getElementById('clear').addEventListener("click", stop);
}

document.getElementById('generate').addEventListener("click", generate);
